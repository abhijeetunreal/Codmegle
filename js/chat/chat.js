    ...
    app.appendMessage = function(msg) {
        ...
        div.innerHTML = `
            <div class="flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[75%] md:max-w-[65%] lg:max-w-[70%]">
                <div class="message-content ${isMe ? 'bg-gradient-message-you text-white rounded-br-sm' : 'bg-gradient-message-stranger text-primary rounded-bl-sm'}">
                    <p class="break-words text-sm md:text-base leading-relaxed font-normal" style="margin: 0; padding: 0.625rem 0.875rem;">${this.escapeHtml(msg.text)}</p>
                    <div class="message-time flex items-center justify-end gap-1 mt-1" style="padding: 0 0.875rem 0.375rem;">
                        <span style="font-size: 0.6875rem; opacity: 0.7;">${timeStr}</span>
                    </div>
                </div>
            </div>
        `;
